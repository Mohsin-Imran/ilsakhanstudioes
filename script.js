...
// Improved internal link + hash handling with header offset
function getHeaderOffset(){
  try{ const header = document.querySelector('.site-header'); return header ? header.offsetHeight + 12 : 88; }catch(e){return 88}
}

function scrollToHashTarget(hash){
  if (!hash) return;
  const id = hash.replace(/^#/, '');
  // map legacy / visual hash names to the visible content block
  const mappedId = (id === 'methodology' && document.getElementById('methodology-text')) ? 'methodology-text' : id;
  ...
  // brief highlight to show the opened section
  target.classList.add('anchor-highlight');
  window.clearTimeout(window.__anchorHighlight);
  window.__anchorHighlight = window.setTimeout(() => target.classList.remove('anchor-highlight'), 2200);
}
  ...
  try{
    ...
    link.addEventListener('click', event => {
      ...
      if (url.pathname === window.location.pathname && url.hash) {
        ...
        setTimeout(() => scrollToHashTarget(url.hash), 40);
        return;
      }

      // cross-page internal navigation: graceful transition
      event.preventDefault();
...
Array.from(document.querySelectorAll('a[href]')).forEach(handleInternalLink);

// ensure hash scroll works reliably on load and when the hash is present
function tryScrollOnLoad(){
  if (window.location.hash) setTimeout(() => scrollToHashTarget(window.location.hash), 120);
  // fallback retry for slow loads or file:// behavior
  setTimeout(() => { if (window.location.hash) scrollToHashTarget(window.location.hash); }, 600);
}

if (document.readyState === 'complete' || document.readyState === 'interactive') tryScrollOnLoad();
else document.addEventListener('DOMContentLoaded', tryScrollOnLoad);

window.addEventListener('hashchange', () => { if (window.location.hash) setTimeout(() => scrollToHashTarget(window.location.hash), 60); });
